...
* A process was prepared to encode categorical features using `CatBoostEncoder()`. The use of `OneHotEncoder()` was not possible because the encoding created too many features, and such a data structure could not be used for model training with limited resources. In this case, `StandardScaler()` was not used, as only tree-based models were planned, for which scaling is not essential.
...
The following model training steps were performed:
...
* A `RandomForestRegressor()` model with specific parameters.
* An `XGBRegressor()` model with specific parameters.
* Search for the best `XGBRegressor()` model using `RandomizedSearchCV()`. The model was tuned sequentially in four steps:
...
* The best obtained `XGBRegressor()` model was retrained in order to retain training process statistics for data analysis.
